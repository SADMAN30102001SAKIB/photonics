...
In addition to the numerical analysis, a machine-learning-assisted prediction approach is introduced to estimate the sensor response for pathogen-specific refractive indices. For this purpose, the confinement loss data obtained over the RI range of 1.33--1.41 are used as the reference dataset. The analyte refractive index is considered as the input parameter, while the resonance wavelength and confinement loss response are considered as the output responses. After training the regression model, the sensor response is predicted for the pathogen RI values of 1.3300, 1.3650, 1.3833, 1.3880, and 1.3921.

The machine-learning model helps to estimate the resonance behavior at pathogen-specific RI values without performing repeated full numerical simulations for every sample. The predicted resonance points follow the same trend as the simulated confinement loss spectra, where higher RI samples show resonance peaks at longer wavelengths. This confirms that the machine-learning-assisted approach can be used as a supporting tool for rapid prediction of pathogen response. Therefore, the combination of numerical sensing analysis and machine learning improves the practical usefulness of the proposed sensor for fast water pathogen identification.

The complete procedure of the machine-learning-assisted pathogen detection approach can be described in four steps. First, the confinement loss spectra are generated for different analyte refractive indices using the optimized DLC-coated PCF-SPR structure. Second, the resonance wavelength and corresponding loss values are extracted from the spectra. Third, the extracted RI-dependent sensing data are used to train a regression-based machine learning model. Finally, the trained model predicts the resonance response for pathogen-specific refractive indices. This approach reduces repeated simulation effort and provides a fast estimation of the optical response of the proposed sensor.

\begin{figure}[!t]
    \centering
    \includegraphics[width=0.48\textwidth]{ML_flowchart}
    \caption{Schematic workflow of the machine-learning-assisted prediction approach for water pathogen detection using the proposed DLC-coated PCF-SPR sensor.}
    \label{fig:ml_flowchart}
\end{figure}
